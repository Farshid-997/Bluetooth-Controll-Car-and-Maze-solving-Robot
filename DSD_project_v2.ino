...
const uint8_t senCount = 8;
uint16_t senVals[senCount];
boolean mode = LOW;
boolean senFlag = LOW;
const byte interPin = 2;
long unsigned int lastPress;
const byte debTime = 40;
boolean ldState = LOW;
unsigned int ldTime = 0;
int lasterr = 0;
byte dataTurn = 0;

void sensorCal(){
  qtr.setTypeAnalog();
  qtr.setSensorPins((const uint8_t []){A0,A1,A2,A3,A4,A5,A6,A7}, senCount);
  qtr.setEmitterPin(3);
...
void sensorData(){
  ...
  uint16_t pos = qtr.readLineBlack(senVals);
  /**/
  for(byte i=0; i<senCount; i++){
  ...
  digitalWrite(LED_BUILTIN, ldState);
  ldTime++;
}
